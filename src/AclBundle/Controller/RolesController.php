...
class RolesController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $roles = $em->getRepository('AclBundle:Roles')->findAll();
    ...
    {
        $role = new Roles();
        $form = $this->createForm('AclBundle\Form\RolesType', $role);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('admin_roles_show', array('id' => $role->getId()));
        }
     ...
     *
     * @Route("/{id}", name="admin_roles_show")
     * @Method("GET")
     */
    ...
    {
        $deleteForm = $this->createDeleteForm($role);
        $editForm = $this->createForm('AclBundle\Form\RolesType', $role);
        $editForm->handleRequest($request);

        if ($editForm->isSubmitted() && $editForm->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('admin_roles_edit', array('id' => $role->getId()));
        }
     ...
     *
     * @Route("/{id}", name="admin_roles_delete")
     * @Method("DELETE")
     */
    ...
    private function createDeleteForm(Roles $role)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('admin_roles_delete', array('id' => $role->getId())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }
